...
DROP DATABASE IF EXISTS `smart_task_platform_benchmark`;

CREATE DATABASE `smart_task_platform_benchmark`
DEFAULT CHARACTER SET utf8mb4
DEFAULT COLLATE utf8mb4_unicode_ci;

USE `smart_task_platform_benchmark`;
...
BEGIN
    ...
    DECLARE v_owner_id BIGINT UNSIGNED DEFAULT 0;
    DECLARE v_project_id BIGINT UNSIGNED DEFAULT 0;
    DECLARE v_creator_id BIGINT UNSIGNED DEFAULT 0;
    DECLARE v_assignee_id BIGINT UNSIGNED DEFAULT 0;
    DECLARE v_task_id BIGINT UNSIGNED DEFAULT 0;

    DECLARE v_comment_user_id BIGINT UNSIGNED DEFAULT 0;
    DECLARE v_root_comment_id BIGINT UNSIGNED DEFAULT NULL;
    DECLARE v_root_comment_user_id BIGINT UNSIGNED DEFAULT NULL;
    ...
    WHILE v_i <= 100 DO
        INSERT INTO `users` (
            ...
            CONCAT('bench_user_', LPAD(v_i, 3, '0')),
            CONCAT('bench_user_', LPAD(v_i, 3, '0'), '@example.com'),
            '$2a$10$JaIDgvOPfLY3Mv0LrJCHX.V4PY6JwA2zD/VP8lMboHDdk4rduV1oK',
            CONCAT('压测用户', LPAD(v_i, 3, '0')),
            CASE
                WHEN MOD(v_i, 5) = 0 THEN CONCAT('https://example.com/avatar/', v_i, '.png')
                ELSE NULL
            END,
            'active',
            CASE
                WHEN MOD(v_i, 3) = 0 THEN DATE_SUB(NOW(), INTERVAL MOD(v_i, 30) DAY)
                ELSE NULL
            END,
            DATE_SUB(NOW(), INTERVAL MOD(v_i, 90) DAY),
            NOW(),
            NULL
        );
    ...
    WHILE v_i <= 20 DO
        -- owner_id 使用 1~20 号用户，保证 owner 也在项目成员中
        SET v_owner_id = v_i;
        ...
        INSERT INTO `projects` (
            ...
            CONCAT('压测项目-', LPAD(v_i, 3, '0')),
            CONCAT('这是压测专用项目，项目序号：', v_i),
            v_owner_id,
            'active',
            DATE_SUB(CURDATE(), INTERVAL MOD(v_i, 30) DAY),
            DATE_ADD(CURDATE(), INTERVAL 180 + v_i DAY),
            DATE_SUB(NOW(), INTERVAL MOD(v_i, 60) DAY),
            NOW(),
            NULL
        );
    ...
    WHILE v_i <= 20 DO
        ...
        WHILE v_j <= 25 DO
            /*
              保证每个项目 owner_id 对应的用户一定是 owner 成员。
              其他成员从用户池中轮询生成，避免同项目重复。
            */
            IF v_j = 1 THEN
                SET v_user_id = v_owner_id;
            ELSE
                SET v_user_id = ((v_i - 1) * 25 + v_j - 1) MOD 100 + 1;

                -- 避免和 owner 重复
                IF v_user_id = v_owner_id THEN
                    SET v_user_id = (v_user_id MOD 100) + 1;
                END IF;
            END IF;
            ...
            INSERT IGNORE INTO `project_members` (
                ...
                v_user_id,
                CASE
                    WHEN v_j = 1 THEN 'owner'
                    WHEN v_j BETWEEN 2 AND 5 THEN 'admin'
                    ELSE 'member'
                END,
                CASE
                    WHEN v_j = 1 THEN NULL
                    ELSE v_owner_id
                END,
                DATE_SUB(NOW(), INTERVAL MOD(v_j, 30) DAY),
                DATE_SUB(NOW(), INTERVAL MOD(v_j, 30) DAY),
                NOW(),
                NULL
            );
    ...
    WHILE v_i <= 20 DO
        ...
        WHILE v_j <= 250 DO
            -- 创建人从当前项目成员池中轮询
            SET v_creator_id = ((v_i - 1) * 25 + MOD(v_j - 1, 25)) MOD 100 + 1;

            -- 负责人从用户池中轮询
            SET v_assignee_id = ((v_i - 1) * 25 + MOD(v_j + 7, 25)) MOD 100 + 1;

            -- 确保部分任务由 owner 创建，便于权限测试
            IF MOD(v_j, 10) = 0 THEN
                SET v_creator_id = v_owner_id;
            END IF;

            -- 构造任务状态
            SET v_status = CASE MOD(v_j, 4)
                WHEN 0 THEN 'done'
                WHEN 1 THEN 'todo'
            ...
            INSERT INTO `tasks` (
                ...
                v_project_id,
                CONCAT('压测任务-', LPAD(v_i, 3, '0'), '-', LPAD(v_j, 4, '0')),
                CONCAT('这是压测专用任务。项目序号：', v_i, '，任务序号：', v_j),
                v_status,
                v_priority,
                v_creator_id,
                v_assignee_id,
                DATE_ADD(NOW(), INTERVAL MOD(v_j, 90) DAY),
                CASE
                    WHEN v_status IN ('in_progress', 'done') THEN DATE_SUB(NOW(), INTERVAL MOD(v_j, 20) DAY)
                    ELSE NULL
                END,
                CASE
                    WHEN v_status = 'done' THEN DATE_SUB(NOW(), INTERVAL MOD(v_j, 10) DAY)
                    ELSE NULL
                END,
                CONCAT('AI 摘要：这是压测任务 ', v_i, '-', v_j, ' 的摘要。'),
                v_j * 100,
                DATE_SUB(NOW(), INTERVAL MOD(v_i * v_j, 120) DAY),
                NOW(),
                NULL
            );
    ...
    WHILE v_task_id <= 5000 DO
        SET v_k = 1;
        SET v_root_comment_id = NULL;
        SET v_root_comment_user_id = NULL;

        WHILE v_k <= 4 DO
            SET v_comment_user_id = MOD(v_task_id + v_k, 100) + 1;
            ...
            INSERT INTO `task_comments` (
                ...
                v_comment_user_id,
                CASE
                    WHEN v_k = 1 THEN NULL
                    ELSE v_root_comment_id
                END,
                CASE
                    WHEN v_k = 1 THEN NULL
                    ELSE v_root_comment_user_id
                END,
                CONCAT(
                    '压测评论内容。任务 ID：',
                    v_task_id,
                ...
                DATE_ADD(DATE_SUB(NOW(), INTERVAL MOD(v_task_id, 60) DAY), INTERVAL v_k MINUTE),
                NOW(),
                NULL
            );
            ...
            -- 记录根评论信息，后续回复使用
            IF v_k = 1 THEN
                SET v_root_comment_id = LAST_INSERT_ID();
                SET v_root_comment_user_id = v_comment_user_id;
            END IF;

            SET v_k = v_k + 1;
        END WHILE;

        SET v_task_id = v_task_id + 1;
    END WHILE;

    COMMIT;

    -- 更新统计信息，帮助 MySQL 优化器选择更合理的执行计划
    ANALYZE TABLE `users`;
    ...
    UNION ALL
    SELECT 'project_members' AS `table_name`, COUNT(*) AS `total` FROM `project_members`
    UNION ALL
    SELECT 'tasks' AS `table_name`, COUNT(*) AS `total` FROM `tasks`
    UNION ALL
    SELECT 'task_comments' AS `table_name`, COUNT(*) AS `total` FROM `task_comments`;
...
-- GET /api/v1/projects/1/tasks?page=10&page_size=10
-- GET /api/v1/projects/1/tasks?status=todo&page=1&page_size=10
-- GET /api/v1/projects/1/tasks?status=in_progress&page=1&page_size=10

-- 推荐压测任务评论：
-- GET /api/v1/tasks/1/comments?page=1&page_size=20
-- GET /api/v1/tasks/100/comments?page=1&page_size=20
-- GET /api/v1/tasks/1000/comments?page=1&page_size=20

-- 推荐权限测试用户：
-- user_id = 1 是 project_id = 1 的 owner
-- user_id = 2~5 大概率是 project_id = 1 的 admin/member
-- user_id = 26 以后可以用来测试非成员权限场景
...
SELECT COUNT(*) AS `project_members_total` FROM `project_members`;
SELECT COUNT(*) AS `tasks_total` FROM `tasks`;
SELECT COUNT(*) AS `task_comments_total` FROM `task_comments`;

-- 检查每个项目的成员数量，应该每个项目约 25 个成员
SELECT
    `project_id`,

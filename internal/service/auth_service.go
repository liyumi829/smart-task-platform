...
type AuthService struct {
	...
	userRepo  AuthUserRepository    // 用户仓库接口
...
func (s *AuthService) Register(ctx context.Context, req *dto.RegisterReq) (*dto.RegisterResp, error) {
	...
	// 更新添加事务
	if err := s.txMgr.Transaction(ctx,
...
func (s *AuthService) RefreshToken(ctx context.Context, req *dto.RefreshTokenReq) (*dto.RefreshTokenResp, error) {
	...
	// 先判断当前用户是否在登录状态
	if err := s.authStore.ValidateRefreshSession(ctx, claims.UserID, claims.SessionID, claims.ID); err != nil {
		switch {
		case errors.Is(err, redispkg.ErrInvalidArgument):
			return nil, ErrInvalidToken

		case errors.Is(err, redispkg.ErrSessionNotFound):
			zap.L().Info("user already logouted...",
				zap.Uint64("user_id", claims.UserID))
...
// AuthUserRepository 认证模块使用的 用户仓储接口
